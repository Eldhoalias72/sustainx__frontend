...
export default function Navigation() {
  ...
  // Changed all items to use full page navigation paths
  const navItems = [
          ...
          <div className="hidden md:flex space-x-8 items-center ml-auto">
            {navItems.map((item) => (
              <Link key={item.name} href={item.path}>
                <motion.span
                  className="text-green-300 hover:text-white transition-colors cursor-pointer"
                  whileHover={{ y: -2 }}
                  whileTap={{ y: 0 }}
                >
                  {item.name}
              ...
              <div className="container mx-auto px-4 py-4">
                <div className="flex flex-col space-y-4">
                  {navItems.map((item) => (
                    <Link key={item.name} href={item.path}>
                      <motion.span
                        className="text-green-300 hover:text-white transition-colors px-4 py-2 block cursor-pointer"
                        whileHover={{ x: 4 }}
